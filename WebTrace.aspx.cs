...
{
    //public struct CHSElement
    //{
    //    public CHSElement(string Key, string Path, string origionalValue, string maskedValue);

    //    public string Key { get; set; }
    //    public string Path { get; set; }
    //    public string origionalValue { get; set; }
        ...
        public partial class WebTrace : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    btnFields.OnClientClick = "viewSession('" + Request.Url.PathAndQuery.ToLower().Replace("jump", "DuckCreek").Replace("webtrace.aspx", "sessionfield.aspx?sessionid='") + ", document.getElementById('" + lstSessions.ClientID + "').value)";
        ...
        private void GetSessions(int maxSessions = 50)
        {
            ...
            if (!ShowError(listSessionsRs, "Session.listSessionsRs"))
            {
                ...
                {
                    if (session.Attribute("sessionID").Value.ToLower().Contains(txtStartsWith.Text.ToLower()))
        ...
        protected void btnGetDownloadLink_Click(object sender, EventArgs e)
        {
            ...
            if (File.Exists(traceFilePath))
            {
                ...
                foreach (string secureFile in securefiles)
                {
                    ...
                    //    client.DownloadFile(Path.Combine(sanitizedTracePath, secureFile), secureFile);
                    //}
        ...
        protected void btnStopTrace_Click(object sender, EventArgs e)
        {
            ...
            if (!ShowError(disconnects, "Diagnostics.disconnectRs"))
            {
                ...
                {
                    lblStatus.Text += " because the size of the trace file has exceeded the maximum limit of " + ConfigurationManager.AppSettings["maxTraceLogSizeMB"] + " MB or the trace has been running longer than the maxinum of " + ConfigurationManager.AppSettings["maxTraceTimeSeconds"] + " seconds.";
